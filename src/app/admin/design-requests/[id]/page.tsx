...
export default async function AdminDesignRequestPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const managePanel = (view: "overview" | "proofs") => (
    <DesignManagePanel
      ...
      rush={request.rush}
      priorityReview={Boolean(request.neededBy && request.neededBy.getTime() - request.createdAt.getTime() < 14 * 24 * 60 * 60 * 1000)}
    ...
    <div className="space-y-8">
      ...
      {linkedOrder && (
        <InboundTracking
          ...
          initial={
            linkedOrder.trackingNumber && linkedOrder.shippedAt && !linkedOrder.inboundTrackingNumber
              ? { trackingNumber: linkedOrder.trackingNumber, carrier: linkedOrder.shipCarrier ?? "Other", destination: "customer" as const }
