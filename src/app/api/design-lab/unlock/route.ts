...
export async function POST(req: Request) {
  ...
  await getDb().update(designLabVisitors).set({ email }).where(eq(designLabVisitors.id, ctx.visitor.id));
  ...
  if (hook) {
    void fetch(hook, {
      ...
      body: JSON.stringify({
        ...
        content: `🧪 AI design lab lead: **${email}** unlocked more generations (used ${ctx.visitor.generations} so far).`,
      }),
    }).catch(() => {});
  }
