...
// Admin-only: waive the one-time $20 embroidery setup fee (repeat hat order
// of a design that already paid it).
export async function POST(req: Request) {
  ...
  let body: { teamOrderId?: string; waived?: boolean } = {};
  try { body = await req.json(); } catch {}
  if (!body.teamOrderId || typeof body.waived !== "boolean") {
  ...
  await db.update(teamOrders).set({ embroideryFeeWaived: body.waived, updatedAt: new Date() }).where(eq(teamOrders.id, body.teamOrderId));
