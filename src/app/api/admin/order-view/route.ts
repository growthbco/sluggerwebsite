...
export async function GET(req: Request) {
  ...
  const rows = items.map((it) => `<tr><td style="padding:6px 10px;border-bottom:1px solid #eee">${esc(it.name)}</td><td style="padding:6px 10px;border-bottom:1px solid #eee;text-align:center">${it.quantity}</td><td style="padding:6px 10px;border-bottom:1px solid #eee;text-align:right">${money(it.unitPriceCents * it.quantity)}</td></tr>`).join("");
  ...
  <body style="font-family:-apple-system,Segoe UI,sans-serif;max-width:640px;margin:24px auto;padding:0 16px;color:#111">
    ...
    <table style="width:100%;margin-top:12px">
      <tr><td style="text-align:right;padding:2px 10px;color:#666">Subtotal</td><td style="text-align:right;padding:2px 10px;width:110px">${money(o.subtotalCents)}</td></tr>
