...
type Hit = { name: string; email: string; phone: string | null; creditCents: number; team: string | null; address: Address; addressAt: Date | null; lastSeen: Date | null };
...
export async function GET(req: Request) {
  ...
  const [fromCustomers, fromOrders, fromTeamOrders, fromDesigns] = await Promise.all([
    ...
    db
      ...
      .limit(10),
    db
      .select({ name: teamOrders.contactName, email: teamOrders.contactEmail, phone: teamOrders.contactPhone, team: teamOrders.teamName, address: teamOrders.shippingAddress, at: teamOrders.createdAt })
      .from(teamOrders)
      .where(or(ilike(teamOrders.contactName, like), ilike(teamOrders.contactEmail, like), ilike(teamOrders.teamName, like)))
      .orderBy(desc(teamOrders.createdAt))
      .limit(10),
    db
      .select({ name: designRequests.contactName, email: designRequests.contactEmail, phone: designRequests.contactPhone, team: designRequests.teamName, at: designRequests.createdAt })
      .from(designRequests)
      .where(or(ilike(designRequests.contactName, like), ilike(designRequests.contactEmail, like), ilike(designRequests.teamName, like)))
  ...
  const add = (name: string | null, email: string | null, phone: string | null, creditCents: number, at: Date | null, address: Address = null, team: string | null = null) => {
    const key = (email ?? "").trim().toLowerCase();
    if (!key) return;
    const addr = address?.postalCode ? address : null;
    const prev = byEmail.get(key);
    if (!prev) {
      byEmail.set(key, { name: name?.trim() || "", email: key, phone, creditCents, team: team?.trim() || null, address: addr, addressAt: addr ? at : null, lastSeen: at });
      return;
    }
    ...
    if (!prev.team && team?.trim()) prev.team = team.trim();
  ...
  for (const t of fromTeamOrders) add(t.name, t.email, t.phone, 0, t.at, t.address, t.team);
  for (const d of fromDesigns) add(d.name, d.email, d.phone, 0, d.at, null, d.team);

  const results = [...byEmail.values()]
    .sort((a, b) => (b.lastSeen?.getTime() ?? 0) - (a.lastSeen?.getTime() ?? 0))
    .slice(0, 8)
    .map(({ name, email, phone, creditCents, team, address }) => ({
      name,
      email,
      phone,
      creditCents,
