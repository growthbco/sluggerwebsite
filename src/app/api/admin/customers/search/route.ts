...
type Hit = { name: string; email: string; phone: string | null; creditCents: number; lastSeen: Date | null };
...
export async function GET(req: Request) {
  ...
  const [fromCustomers, fromOrders, fromTeamOrders, fromDesigns] = await Promise.all([
    db
      ...
      .limit(10),
    db
      .select({ name: orders.customerName, email: orders.customerEmail, at: orders.createdAt })
      .from(orders)
      .where(or(ilike(orders.customerName, like), ilike(orders.customerEmail, like)))
      .orderBy(desc(orders.createdAt))
      .limit(10),
    db
      .select({ name: teamOrders.contactName, email: teamOrders.contactEmail, phone: teamOrders.contactPhone, at: teamOrders.createdAt })
      .from(teamOrders)
  ...
  const add = (name: string | null, email: string | null, phone: string | null, creditCents: number, at: Date | null) => {
    const key = (email ?? "").trim().toLowerCase();
    if (!key) return;
    const prev = byEmail.get(key);
    if (!prev) {
      byEmail.set(key, { name: name?.trim() || "", email: key, phone, creditCents, lastSeen: at });
      return;
    }
  ...
  for (const o of fromOrders) add(o.name, o.email, null, 0, o.at);
  for (const t of fromTeamOrders) add(t.name, t.email, t.phone, 0, t.at);
  ...
  const results = [...byEmail.values()]
    ...
    .slice(0, 8)
    .map(({ name, email, phone, creditCents }) => ({ name, email, phone, creditCents }));
